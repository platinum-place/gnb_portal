...
class Reportes extends BaseController
{
    public function index()
    {
        ...
        $sheet->setCellValue('D2', 'EMISIONES PLAN ' . $this->request->getPost("plan"));
        $sheet->setCellValue('D4', 'Generado por:');
        $sheet->setCellValue('E4', session("usuario"));
        $sheet->setCellValue('D5', 'Desde:');
        $sheet->setCellValue('E5', $this->request->getPost("desde"));
        $sheet->setCellValue('D6', 'Hasta:');
        $sheet->setCellValue('E6', $this->request->getPost("hasta"));
        ...
        $emisiones = $libreria->searchRecordsByCriteria("Quotes", $criterio);
            ...
            case 'Auto':
                ...
                foreach ($emisiones as $emisiones => $emision) {
                    if (
                        ...
                        and
                        ($emision->getFieldValue('Plan') == "Mensual Full" or $emision->getFieldValue('Plan') == "Anual Full")
                    ) {
                        //valores de la tabla
            ...
            case 'Vida':
                ...
                foreach ($emisiones as $emisiones => $emision) {
                    if (
                        ...
                        $emision->getFieldValue('Quote_Stage') == "Emitida"
                        and
                        $emision->getFieldValue('Plan') == "Vida"
                    ) {
                        //valores de la tabla
                        $sheet->setCellValue('A' . $pos, $cont);
                        $sheet->setCellValue('B' . $pos, $emision->getFieldValue('Contact_Name')->getLookupLabel());
            ...
            case 'Vida/Desempleo':
                ...
                foreach ($emisiones as $emisiones => $emision) {
                    if (
                        ...
                        $emision->getFieldValue('Quote_Stage') == "Emitida"
                        and
                        $emision->getFieldValue('Plan') == "Vida/Desempleo"
                    ) {
                        //valores de la tabla
                        $sheet->setCellValue('A' . $pos, $cont);
                        $sheet->setCellValue('B' . $pos, $emision->getFieldValue('Contact_Name')->getLookupLabel());
            ...
            case 'Seguro Incendio Hipotecario':
                ...
                foreach ($emisiones as $emisiones => $emision) {
                    if (
                        ...
                        $emision->getFieldValue('Quote_Stage') == "Emitida"
                        and
                        $emision->getFieldValue('Plan') == "Seguro Incendio Hipotecario"
                    ) {
                        //valores de la tabla
                        $sheet->setCellValue('A' . $pos, $cont);
                        $sheet->setCellValue('B' . $pos, $emision->getFieldValue('Contact_Name')->getLookupLabel());
                        ...
                        $sheet->setCellValue('N' . $pos, $emision->getFieldValue('Construcci_n'));
